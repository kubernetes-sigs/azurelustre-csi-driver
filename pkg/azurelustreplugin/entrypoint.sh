...
function load_lnet() {
  ...
  # /sys/module check, timeout-bounded lnetctl: avoids the lsmod|grep SIGPIPE
  # pitfall and stops a wedged call deferring teardown (see add_net_interfaces).
  if [[ -d /sys/module/lnet ]]; then
    # Capture-then-match (SIGPIPE pitfall); timeout-bound the lnetctl calls.
...
function reconcile_lnet() {
  ...
  # /sys/module check: avoids the lsmod|grep SIGPIPE false-negative under pipefail.
  if [[ ! -d /sys/module/lnet ]]; then
  ...
  # Capture-then-match (SIGPIPE pitfall); timeout-bound the lnetctl call.
  local tcp_net_show
  tcp_net_show=$(timeout 10 lnetctl net show --net tcp 2>/dev/null || true)
  if ! grep -q interfaces <<<"${tcp_net_show}"; then
    echo "$(date -u) LNet tcp network missing interfaces; reconfiguring."
    timeout 10 lnetctl lnet configure || true
  fi
  ...
  add_net_interfaces quiet || true
}

# teardown_lnet unloads the Lustre/LNet kernel modules when the loader sidecar
# is terminating so a driver upgrade does not leave stale modules resident on
# the host. It runs from the SIGTERM/SIGINT trap (not a preStop hook) so it
# shares the loader's PID 1 with the reconcile loop: bash never runs the trap
# concurrently with an in-flight reconcile, so teardown cannot race the loop
# re-adding the NI the way a separate preStop process could.
#
# lustre_rmmod does the whole job in one call: it unloads the client stack in
# dependency order and drops LNet (the "which"-gated "lnetctl lnet unconfigure"
# inside the upstream script now runs because the image ships "which"). It is
# timeout-bounded so a wedged unload cannot consume the whole grace period. A
# non-zero exit means a module could not be removed -- most likely a filesystem
# is still mounted -- which the operator needs to know about before the driver
# is replaced, so it is surfaced as a WARNING.
function teardown_lnet() {
  ...
  if timeout 15 lustre_rmmod; then
    ...
    echo "$(date -u) WARNING: Lustre kernel modules could not be unloaded on teardown (a filesystem is likely still mounted -- drain workloads before removing the driver); modules will remain resident until the node reboots."
  fi
}
...
function validate_client_version() {
  ...
  # OS-specific package version naming convention.
  if [[ "${osFamily}" == "azurelinux" ]]; then
    # Azure Linux RPM packages use underscores: 2.16.1_21_g153e389
    pkgVersion="${LUSTRE_VERSION}_${CLIENT_SHA_SUFFIX//-/_}"
  else
...
# prepare_depmod_stub creates an empty module dir so the Ubuntu lustre-client
# deb's postinst `depmod -a` does not FATAL in the driver container (which ships
# no kernel package and does not mount the host module tree). No-op elsewhere.
...
# evict_old_lustre is the loader's on-failure hook: it unloads running modules
# and removes existing Lustre client packages so a retry can install cleanly.
# The driver never calls this -- the modules belong to the kernel and were
# loaded by the loader sidecar.
...
# outcome. An install failure is logged but does not abort the container: the
# loader then fails at load_lnet, and the driver still execs.
...
function run_loader() {
  ...
  while true; do
    ...
    wait $! || true
    reconcile_lnet || echo "$(date -u) reconcile_lnet iteration failed; will retry next interval."
  done
  # Not reached.
}

# run_driver installs only the userspace utils, then execs the CSI driver.
function run_driver() {
...
# Refresh the CA store for every role so outbound HTTPS works: package installs
# from packages.microsoft.com (loader/driver) and the controller's Azure API calls.
update_ca_trust
...
# Role selects how this container participates in the node plugin. It is
# REQUIRED; an unset or unknown value is a deployment error, not a silent no-op.
#   loader      - sidecar: install the full client (kmod+kernel+utils), load the
#                 kernel modules, configure LNet, then run a reconcile loop.
#                 Never execs the CSI driver.
#   driver      - install ONLY the userspace utils (kernel-agnostic; tracks the
#                 user-overridable Lustre SHA) then exec the CSI driver. Relies
#                 on the loader sidecar having already loaded the kernel modules
#                 into the shared host kernel.
#   controller  - no kernel-module work: skip install/load and just exec.
