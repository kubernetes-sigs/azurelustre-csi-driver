...
verbose_out = sys.stderr if args.verbose else open(os.devnull, "w", encoding="utf-8")  # pylint: disable=consider-using-with
...
def file_passes(filename, refs, regexs):
    ...
    if len(ref) > len(data):
        print(f'File {filename} smaller than reference ({len(data)} < {len(ref)})',
              file=verbose_out)
        return False
...
def is_autogenerated(data, regexs):
    ...
    return bool(p.match(data))

def file_extension(filename):
    return os.path.splitext(filename)[1].split(".")[-1].lower()

skipped_dirs = ['Godeps', 'third_party', '_gopath', '_output', '.git', '.jj',
...
def get_regexs():
    ...
    regexs["date"] = re.compile(f'({"|".join(str(y) for y in years)})')
    # strip // +build \n\n build constraints
